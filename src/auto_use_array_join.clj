(ns auto-use-array-join
  (:require
    [clojure.string :as str]))

(defn create-struct []
  {:from [[[:metabase.util.honey-sql-2/identifier :table ["default" "test"]]]]
   :select [
            [
             [:metabase.util.honey-sql-2/typed
              [:metabase.util.honey-sql-2/identifier :field ["default" "test" "tags"]]
              {:database-type "array(string)"}]
             [[:metabase.util.honey-sql-2/identifier :field-alias ["tags"]]]
             ]

            [:%count.*
             [[:metabase.util.honey-sql-2/identifier :field-alias ["count"]]]
             ]

            [
             [:max
              [:metabase.util.honey-sql-2/typed
               [:metabase.util.honey-sql-2/identifier :field ["default" "test" "_id"]]
               {:database-type "string"}
               ]
              ]
             [[:metabase.util.honey-sql-2/identifier :field-alias ["max"]]]
             ]
            ]
   :group-by [
              [:metabase.util.honey-sql-2/typed
...
(defn -main
  [& args]
  (let [struct (create-struct)
        struct-replaced (auto-use-array-join struct)]
    (println struct)
    (println)
    (println struct-replaced)
    )
  (println "ok"))
